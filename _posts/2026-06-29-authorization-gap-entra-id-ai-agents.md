...
1. **Target Single-Tenant Service Principals:** Isolate AI app registrations completely and enforce explicit location boundaries via Conditional Access for Workload Identities.
2. **Implement Managed Identities:** Wherever possible, eliminate client secrets or certificates entirely by migrating background workloads to Azure Managed Identities, removing standing credential export risks.
